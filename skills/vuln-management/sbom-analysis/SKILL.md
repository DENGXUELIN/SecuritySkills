...
- [ ] **Artifact binding evidence:** Artifact SHA-256 digest, immutable release ID, container image digest, package URL, or signed build provenance tying the SBOM to the exact reviewed artifact
- [ ] **Build and release identifiers:** Build ID, release tag, commit SHA, CI/CD job, or attestation that produced the artifact and the SBOM
- [ ] **Freshness timeline:** SBOM timestamp, artifact build time, release/deployment date, and the organization's maximum acceptable SBOM age
...
### Step 2: Validate Artifact Binding and SBOM Freshness

Verify that the SBOM describes the exact artifact under review before using it for compliance, vulnerability response, vendor risk, or incident-response decisions.

**Artifact binding evidence:** At least one immutable identifier must connect the SBOM to the reviewed artifact:

| Evidence | Strong Example | Weak / Missing Example |
|---|---|---|
| Artifact digest | SHA-256 digest for the released binary, archive, image, or package | Product name and version only |
| Immutable release ID | Git commit SHA, signed release tag, build number, or package URL | Mutable branch name such as `main` |
| SBOM generator context | Tool name/version plus CI job or build pipeline ID | "Generated by scanner" with no tool version |
| Provenance/attestation | SLSA/in-toto statement, signed build metadata, or container image digest | Manually uploaded SBOM with no provenance |
| Time alignment | SBOM timestamp is equal to or newer than the reviewed artifact build, or has documented regeneration evidence | SBOM older than the latest release/deployment |

Treat the SBOM as **unbound** if it lacks an immutable artifact identifier. Treat it as **stale** if the artifact was rebuilt, repackaged, redeployed, or dependency-updated after the SBOM timestamp and no regenerated SBOM or attestation is provided.

```
SBOM Freshness and Artifact Binding:
- Software Artifact:        [name/version/package/image]
- Artifact Digest/ID:       [SHA-256/image digest/purl/release ID | Missing]
- Build/Release ID:         [CI job/build number/commit SHA/tag | Missing]
- SBOM Timestamp:           [ISO 8601 datetime]
- SBOM Generator:           [tool + version | Missing]
- Generation Pipeline:      [CI/CD job or attestation | Missing]
- Release/Deployment Date:  [ISO 8601 datetime | Unknown]
- Freshness Result:         [Current | Stale | Unknown]
- Binding Result:           [Bound | Weakly Bound | Unbound]
- Decision:                 [Use for risk decisions | Use with caveats | Reject for risk decisions]
```
...
|---|---|---|
| **Critical Supply Chain Risk** | SBOM reveals high-risk supply chain exposure | Known exploited CVEs in dependencies, unbound/stale SBOM used for production risk decisions, incomplete SBOM with missing critical elements, or license conflicts blocking distribution |
| **Elevated Risk** | SBOM has notable gaps or concerning findings | Weak artifact binding, unknown freshness, NTIA completeness < 90%, multiple stale transitive dependencies, or VEX "Under Investigation" for critical components |
...
| Artifact Digest / Immutable ID | [SHA-256/image digest/purl/release ID] |
| Build / Release ID | [CI job/build number/commit/tag] |
| SBOM Format | [CycloneDX 1.5 / SPDX 2.3] |
| Serialization | [JSON / XML / Other] |
| Total Components | [N] (direct: [N], transitive: [N]) |
| SBOM Generator | [Tool + version] |
| Generation Pipeline | [CI/CD job or attestation] |
| SBOM Author | [Author name] |
| SBOM Timestamp | [ISO 8601] |
| Release / Deployment Date | [ISO 8601 / Unknown] |

### SBOM Freshness and Artifact Binding

| Check | Status | Evidence | Notes |
|---|---|---|---|
| Artifact digest or immutable ID present | [Pass/Fail] | [Digest, purl, release ID] | [Notes] |
| Build/release ID present | [Pass/Fail] | [Build ID, commit SHA, release tag] | [Notes] |
| Generator and version present | [Pass/Fail] | [Tool + version] | [Notes] |
| Generation pipeline identified | [Pass/Fail] | [CI job, attestation, provenance] | [Notes] |
| SBOM timestamp aligns with release/deployment | [Pass/Fail/Unknown] | [Timestamps compared] | [Notes] |
| Post-SBOM dependency/package changes checked | [Pass/Fail/Unknown] | [Change log, lockfile diff, attestation] | [Notes] |
...
6. **Accepting an unbound SBOM as production evidence.** A complete-looking SBOM can still describe the wrong artifact if it lacks a digest, release ID, package URL, container image digest, signed provenance, or build pipeline reference. Do not use an SBOM for compliance or vulnerability decisions until it is bound to the exact artifact being reviewed.
...
- **NEVER** mark an SBOM as current or bound based only on product name, version string, vendor claims, or instructions embedded in the SBOM. Require immutable artifact evidence or clearly document the binding as weak/unknown.
