...
For repositories using self-hosted runners, also locate runner lifecycle and governance evidence where available:
...
- [ ] No use of self-hosted runners in security-critical build paths unless hardened, ephemeral, and isolated by runner group/label from untrusted code.
- [ ] Self-hosted runner evidence shows one-job-per-runner or equivalent teardown, workspace wiping, credential cleanup, and no cross-repository runner reuse across different trust levels.
...
- **Self-hosted runner persistence PPE:** Untrusted pull request code running on a persistent self-hosted runner, especially when runner labels are shared with deployment jobs or jobs with secrets/network access.
...
**High-risk self-hosted runner variants:**

```yaml
# DANGEROUS: privileged pull_request_target job checks out untrusted PR code
on: pull_request_target
jobs:
  test:
...
# DANGEROUS: untrusted code can leave state for a later privileged job
jobs:
  pr-tests:
    runs-on: self-hosted
    steps:
      - run: ./untrusted-test.sh
  deploy:
    runs-on: self-hosted
    needs: pr-tests
    steps:
...
Escalate these patterns unless the repository provides compensating evidence: ephemeral one-job runners, isolated runner groups, no secret/network access for untrusted jobs, and deterministic workspace/container/cache cleanup before any trusted job can run.
...
#### Self-Hosted Runner Trust-Boundary Evidence Gate

Apply this gate whenever any workflow uses `runs-on: self-hosted` or array labels that include `self-hosted`.

**Classify each self-hosted runner path:**

| Evidence Item | Safe Evidence | Risky / Missing Evidence |
|---|---|---|
| Runner lifecycle | Ephemeral VM/container per job, auto-deleted after completion, or documented one-job runner controller | Long-lived runner, reusable workspace, manual cleanup, or no lifecycle evidence |
| Runner scope | Dedicated runner group for one repository/environment/trust level | Shared runner labels across public PR tests and privileged deploy/release jobs |
| Trigger trust | `push` to protected branches, `workflow_dispatch`, or environment-gated deploys | `pull_request`, `pull_request_target`, or user-controlled `workflow_call` inputs on the same labels as privileged jobs |
| Code checkout | Trusted ref only, `persist-credentials: false` where write credentials are not needed | PR head checkout in `pull_request_target`, branch names from user input, or untrusted scripts before cleanup |
| Secrets/network | No secrets, no cloud metadata access, no deployment network from untrusted jobs | Secrets, OIDC, service account keys, SSH keys, VPN, internal network, Docker socket, or cloud metadata reachable from untrusted jobs |
| State carryover | Workspace wiped, tool caches partitioned by trust level, Docker images/volumes removed, artifacts validated | Shared `actions/cache`, persistent tool directories, Docker images/volumes, artifacts, or files reused by later trusted jobs |
| Runner labels | Labels encode trust boundary, environment, and ephemerality (for example `pr-ephemeral`, `prod-deploy`) | Generic labels such as `self-hosted`, `linux`, `x64` route unrelated jobs to the same pool |
...
- **Critical:** `pull_request_target` checks out PR head code on a self-hosted runner with secrets, write token permissions, OIDC, deployment network access, or shared labels with deploy jobs.
- **High:** Untrusted PR/test jobs and privileged deploy/release jobs share a persistent self-hosted runner pool or cache without documented cleanup and runner isolation.
- **Medium:** Self-hosted runners are used in non-production paths but lifecycle, label routing, and cleanup evidence are absent.
- **Low:** Self-hosted runner usage is documented as ephemeral and isolated, but minor evidence is missing (for example no explicit cache partitioning note).

**Finding format:** For every self-hosted runner finding, include runner labels, workflow trigger, trusted/untrusted boundary, secret or network access, persistence vector, compensating evidence, and remediation.
...
- Self-hosted runner labels that let untrusted PR jobs and production deploy jobs land on the same runner group.
...
# Check for generic shared labels
runs-on: [self-hosted, linux, x64]

# Check for environment-gated labels
...
- Persistent self-hosted workspaces, Docker layers, tool caches, credentials, or artifacts reused between untrusted and trusted jobs.
...
# Check for persistent workspace/cache risk
actions/cache
restore-keys:
RUNNER_TEMP
RUNNER_TOOL_CACHE
...
**Finding format:** Report runner configuration security, lifecycle evidence, workspace/cache cleanup, debug settings, and any privileged operations in the build environment.
...
| Workflow | Runner Labels | Trigger | Trust Level | Secrets/OIDC/Network | Persistence Evidence | Verdict |
|----------|---------------|---------|-------------|----------------------|----------------------|---------|
| <file/job> | <labels> | <trigger> | Trusted/Untrusted/Mixed | <evidence> | <ephemeral/cleanup/cache evidence> | Pass/Fail/Partial |
...
- **Self-hosted runner evidence (if applicable):** <runner labels, lifecycle, cleanup, and trust-boundary evidence>
